...
class CartPoleEnv(gym.Env[np.ndarray, Union[int, np.ndarray]]):
    ...
    * v0: Initial versions release.
    ...
    def render(self):
        ...
        def draw_gradient(surf, color1, color2):
            rect = surf.get_rect()
            for y in range(rect.height):
                color = (
                    color1[0] + (color2[0] - color1[0]) * y // rect.height,
                    color1[1] + (color2[1] - color1[1]) * y // rect.height,
                    color1[2] + (color2[2] - color1[2]) * y // rect.height
                )
                pygame.draw.line(surf, color, (0, y), (rect.width, y))

        # Colors for the gradient
        sky_color_top = (135, 206, 235)  # Light sky blue
        sky_color_bottom = (0, 191, 255) # Deep sky blue

        self.surf = pygame.Surface((self.screen_width, self.screen_height))
        draw_gradient(self.surf, sky_color_top, sky_color_bottom)
        ...
        gfxdraw.aapolygon(self.surf, cart_coords, (255, 165, 0))
        gfxdraw.filled_polygon(self.surf, cart_coords, (255, 165, 0))
